...
| `plugdev config list\|open\|get\|set\|watch\|unwatch` | Edit live plugin configs |
...
```powershell
plugdev config list
plugdev config open                  # config.yml
...
The TUI has the same workflow under **Config files**. `Enter` opens a file and `Space` toggles reload-on-save. `config get` / `config set` change dotted keys without leaving the terminal (agents use the same commands via MCP).
...
**v0.12.8** — keyed live config get/set plus MCP config tools. See [CHANGELOG.md](CHANGELOG.md).
