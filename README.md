...
Later: `plugdev update` (or `update.auto: true` in `plugdev.yml` / `~/.plugdev/config.yml`).
...
Server + Via* + deps cache under `~/.plugdev/`. Project run folder: `.plugdev/run/`. Client joins `127.0.0.1:25565`. Edit `src/` → save → safe reload (or hotswap when enabled). Edit an allowlisted live plugin config → reload without rebuilding the JAR.
...
| `plugdev config list\|open\|get\|set\|editor\|watch\|unwatch` | Edit live plugin configs |
...
PlugDev can open the config files generated by the plugin inside its persistent dev server:

```powershell
plugdev config list
plugdev config open                  # config.yml (preferred editor)
plugdev config open --editor cursor
plugdev config open --editor notepad
plugdev config editor cursor         # remember preference in plugdev.yml
plugdev config open lang/en_US.yml
plugdev config get --key value
plugdev config set --key value --value two
plugdev config set nested.yml --key feature.enabled --value true
plugdev config watch lang/en_US.yml
...
```yaml
dev:
  configEditor: cursor   # auto | cursor | code | notepad | system
...
These paths are relative to `.plugdev/run/plugins/<PluginName>/`. They are live dev-server copies; PlugDev never overwrites the defaults in `src/main/resources`. Only allowlisted files trigger a reload, so player data and plugin-written caches do not restart the plugin.

## Headless control (optional)

MCP (`npx -y @plugdev/mcp`) exposes the same loop for headless control: doctor, build, sync, server, RCON, modules, dependencies, live configs, cache, and clean. Interactive users can ignore it and use `plug run`.
...
**v1.0.3** — stable local Paper loop + live config UX. Update with `plugdev update` (or set `update.auto: true`). After 1.0, prefer additive `plugdev.yml` changes. See [CHANGELOG.md](CHANGELOG.md).
